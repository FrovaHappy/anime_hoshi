...
export async function getUser(req: Request, res: Response<JsonResponse>) {
  ...
  if (!decoded) return res.status(403).json({ code: 403, message: 'error decoding token', contents: null })
  const response = await findUser(decoded.username)
  if (!response) return res.status(403).json({ code: 403, message: 'resource not found', contents: null })
...
export async function updateUser(req: Request, res: Response<JsonResponse>) {
  ...
  if (!user) return res.status(403).json({ code: 403, message: 'error updating user', contents: null })
