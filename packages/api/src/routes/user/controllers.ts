...
export async function signin (req: Request, res: Response<JsonResponse>) {
  ...
  if (userValidate == null) return res.status(403).json({ code: 403, message: 'Invalid username or password', contents: null })
  const newToken = createUserToken({ username: userValidate.username, id: userValidate.id, roles: userValidate.roles })
  return res.status(200).json({ code: 200, message: 'user singIn successfully', contents: { newToken } })
}
export async function getUser (req: Request, res: Response<JsonResponse>) {
  const { authorization } = req.headers
  const decoded = auth.decodedToken(authorization ?? '')
  if (decoded == null) return res.status(403).json({ code: 403, message: 'error decoding token', contents: null })
  const response = await findUser(decoded.username)
  if (response == null) return res.status(403).json({ code: 403, message: 'resource not found', contents: null })
  const user = { username: response.username, roles: response.roles }
  const newToken = auth.createToken(user, '24h')
  return res.status(200).json({ code: 200, message: 'user authenticated', contents: { newToken, ...user } })
}
export async function updateUser (req: Request, res: Response<JsonResponse>) {
  ...
  if (user == null) return res.status(403).json({ code: 403, message: 'error updating user', contents: null })
  const userData = { username: user.username, roles: user.roles }
  const newToken = auth.createToken(userData, '24h')
  return res.status(200).json({ code: 200, message: 'user updated', contents: { newToken, ...userData } })
}
