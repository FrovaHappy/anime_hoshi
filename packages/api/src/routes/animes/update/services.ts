...
async function saveRedirectId(putAnime: UpdateAnimeBodyType) {
  const anime = await animeDb.findOne({ search: `${putAnime.id}`, searchType: 'id' })
  ...
  return await animeDb.updateOne({ anime, filter: { id: anime.id } })
}
async function createNewAnime(putAnime: UpdateAnimeBodyType) {
  ...
  const newAnime: Anime = {
    ...dataAnilist,
    titles: [putAnime.title],
    lastUpdate: Date.now(),
    pages: {
      [putAnime.namePage]: {
        episodes: episodes ?? [],
        lang: 'JP',
        namePage: putAnime.namePage,
        redirectId: 0,
  ...
  return await animeDb.updateOne({ anime: newAnime, filter: { id: dataAnilist.id } })
}
...
async function updateAnime(anime: Anime, putAnime: UpdateAnimeBodyType) {
  ...
  pages[putAnime.namePage] = page
  anime.pages = pages
  anime.titles = [...new Set([putAnime.title, ...anime.titles])]
  return await animeDb.updateOne({ anime, filter: { id: anime.id } })
}
export async function createRedirectId(putAnime: UpdateAnimeBodyType) {
  const animeRedirect = await animeDb.findOne({ search: `${putAnime.redirectId}`, searchType: 'id' })
...
export async function updateGeneral(putAnime: UpdateAnimeBodyType) {
  const findId = await animeDb.findOne({ search: `${putAnime.id}`, searchType: 'id' })
