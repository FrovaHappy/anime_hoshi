...
import { createUserToken, servicesCreateUser, servicesUpdateUser } from '../services/usersServices'
...
export async function signin(req: Request, res: Response) {
  ...
  return res.status(200).json({ code: 200, message: 'user singin successfully', contents: { token } })
}
...
export async function updateUser(req: Request, res: Response) {
  ...
  return res.status(200).json({ code: 200, message: 'user updated', contents: { user } })
}