...
// MODULE FORMATTED
function stringToObject(s: string | undefined) {
  try {
...
// MODULE MAIN ****************************************************************
function canSendMessage(inAwait: InAwait, notificationSetting: NotificationsInAired, currentTime: number) {
 ...
 */
function addInAwaitToCollection(
  push: PayloadAnimeNof[],
  inAwaitCollection: NotificationInAwait,
  expireIn: number,
  currentTime: number
) {
...
function buildMessage(inAwait: InAwait) {
  ...
  const options: Notification | object = {
    body,
...
self.addEventListener('push', async (e: any) => {
  ...
  let notificationsInAwait: NotificationInAwait =
    stringToObject((await DBLocal.get('notificationsInAwait'))?.value) ?? []
